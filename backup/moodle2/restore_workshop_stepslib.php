...
class restore_workshop_activity_structure_step extends restore_activity_structure_step {
    ...
    // Indicate if it is a backup of an old workshop (before wizard)
    protected $oldworkshop = false;
    ...
    protected function after_execute() {
        ...
        // If it was the backup of an old workshop some fields are missing.
        if ($this->oldworkshop) {
            $missingdata = new stdClass();
            $missingdata->id = $this->get_new_parentid('workshop');
            $missingdata->assesswithoutsubmission = 1;

            $sql = "SELECT 1
                      FROM {workshop_submissions} s
                      JOIN {workshop_assessments} a ON (s.id = a.submissionid)
                     WHERE s.workshopid = :workshopid
                       AND s.authorid = a.reviewerid";
            if ($DB->record_exists_sql($sql, array('workshopid' => $missingdata->id))) {
                $missingdata->assessmenttype = 3;
            }
            $DB->update_record('workshop', $missingdata);
        }
