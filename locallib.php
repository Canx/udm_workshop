...
class udmworkshop {
    ...
    public function prepare_assessment(stdClass $record, $form, array $options = array()) {
        ...
        $assessment->realsubmission = $this->allowsubmission;
    ...
    public function delete_submission(stdclass $submission, $keepallocation = false) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($this->context->id, 'mod_udmworkshop', 'submission_content', $submission->id);
        $fs->delete_area_files($this->context->id, 'mod_udmworkshop', 'submission_attachment', $submission->id);
    ...
    public function delete_assessment($id) {
        ...
        if (is_array($id)) {
            ...
            foreach ($id as $itemid) {
                $fs->delete_area_files($this->context->id, 'mod_udmworkshop', 'overallfeedback_content', $itemid);
                $fs->delete_area_files($this->context->id, 'mod_udmworkshop', 'overallfeedback_attachment', $itemid);
            }
            $DB->delete_records_list('workshop_assessments', 'id', $id);

        } else {
            $DB->delete_records('workshop_grades', array('assessmentid' => $id));
            $fs->delete_area_files($this->context->id, 'mod_udmworkshop', 'overallfeedback_content', $id);
            $fs->delete_area_files($this->context->id, 'mod_udmworkshop', 'overallfeedback_attachment', $id);
    ...
    public function switch_phase($newphase) {
        ...
        $event = \mod_udmworkshop\event\phase_switched::create($eventdata);
        $event->trigger();
        return true;
    }
    ...
    public function get_gradebook_grades($userid) {
        ...
        if (!is_null($grades->submissiongrade) or !is_null($grades->assessmentgrade)) {
            $grades->realsubmission = $this->allowsubmission;
            return $grades;
        }
    ...
    protected function aggregate_grading_grades_process(array $assessments, $timegraded = null) {
        ...
        if (grade_floats_different($finalgrade, $current)) {
            ...
            if (is_null($agid)) {
                ...
                $event = \mod_udmworkshop\event\assessment_evaluated::create($params);
                $event->trigger();
            } else {
                $record = new stdclass();
                $record->id = $agid;
                ...
                $event = \mod_udmworkshop\event\assessment_reevaluated::create($params);
                $event->trigger();
            }
        }
    }
    ...
    /**
     * Check if asubmission's author can be viewed by reviewer.
     *
     * @return bool True if user has capability
     */
    public function can_view_author_names() {
        if ($this->allowsubmission) {
            return has_capability('mod/workshop:viewauthornames', $this->context);
        } else {
            return true;
        }
    }
...
class workshop_user_plan implements renderable {
    ...
    /**
     * Returns tasks associated to instruction for reviewers
     *
     * @param int $userid User ID
     * @return array
     */
    protected function add_tasks_instructreviewers($userid) {
    ...
    /**
     * Returns tasks associated to peers allocation
     *
     * @param int $userid User ID
     * @return array
     */
    protected function add_tasks_allocate($userid) {
        global $DB;
        ...
        if (has_capability('mod/workshop:allocate', $workshop->context, $userid)) {
            ...
            $numofsubmissions = 0;
            $numwaitingtoassess = 0;
            $numshouldnotassess = 0;
            if ($workshop->allowsubmission) {
...
class workshop_submission_summary extends workshop_submission_base implements renderable {
    ...
    public $url;
    /** @var bool  indicate if submission is real*/
    public $realsubmission;
    ...
    protected $fields = array(
        ...
        'authorimagealt', 'authoremail', 'realsubmission');
}
...
class workshop_submission extends workshop_submission_summary implements renderable {
    ...
    protected $fields = array(
        ...
        'authormiddlename', 'authoralternatename', 'authorpicture', 'authorimagealt', 'authoremail', 'realsubmission');
}
...
class workshop_assessment extends workshop_assessment_base implements renderable {
    ...
    public function get_overall_feedback_content() {
        ...
        $content = file_rewrite_pluginfile_urls($this->feedbackauthor, 'pluginfile.php', $this->workshop->context->id,
            'mod_udmworkshop', 'overallfeedback_content', $this->id);
    ...
    public function get_overall_feedback_attachments() {
        ...
        $attachments = array();
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->workshop->context->id, 'mod_udmworkshop', 'overallfeedback_attachment', $this->id);
        foreach ($files as $file) {
            ...
            $fileurl = moodle_url::make_pluginfile_url($this->workshop->context->id, 'mod_udmworkshop',
                'overallfeedback_attachment', $this->id, $filepath, $filename, true);
            $previewurl = new moodle_url(moodle_url::make_pluginfile_url($this->workshop->context->id, 'mod_udmworkshop',
