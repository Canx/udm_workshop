...
function udmworkshop_print_recent_activity($course, $viewfullnames, $timestart) {
    ...
    foreach ($rs as $activity) {
        ...
        if ($activity->submissionmodified > $timestart and empty($submissions[$activity->submissionid])) {
            ...
            if ($activity->authorid == $USER->id || !$activity->allowsubmission || has_capability('mod/workshop:viewauthornames', $context)) {
...
function udmworkshop_print_recent_mod_activity($activity, $courseid, $detail, $modnames, $viewfullnames) {
    ...
    if ($activity->subtype == 'submission') {
        ...
        if ($detail) {
            ...
            $url = new moodle_url('/mod/udmworkshop/view.php', array('id'=>$activity->cmid));
        ...
        $url = new moodle_url('/mod/udmworkshop/submission.php', array('cmid'=>$activity->cmid, 'id'=>$activity->content->id));
    ...
    if ($activity->subtype == 'assessment') {
        ...
        echo html_writer::start_tag('div', array('class'=>'title'));
        $url = new moodle_url('/mod/udmworkshop/assessment.php', array('asid'=>$activity->content->id));
